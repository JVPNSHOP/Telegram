...
async def menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Send menu to the user (used as fallback for general messages)
    if update.message:
        await update.message.reply_text(
...
async def handle_file_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Some types (photo) may not have file_name
    name = getattr(file, "file_name", None) or f"{file_type}_{file.file_id}"
...
async def send_file_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if result:
            ...
            # send_document accepts file_id stored in DB
...
def main():
    ...
    # -------------------------
    # Register conversation handlers FIRST so they can catch callback queries
    # -------------------------
    ...
    # -------------------------
    # Generic callback handler placed AFTER conversation handlers so they don't get preempted
    # -------------------------
    application.add_handler(CallbackQueryHandler(button_handler))

    # -------------------------
    # Fallback: show menu on any plain message (so menu appears without /start)
    # Placed after conversations so upload flows still work
    # -------------------------
    application.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, menu_command))
