#!/usr/bin/env python3
"""
Complete Telegram Bot Setup and Runner
This single script handles:
1. System setup (python3-venv installation)
...
def check_and_install_dependencies():
    ...
    try:
        subprocess.run(['dpkg', '-l', 'python3-venv'], check=True, capture_output=True)
        print("✓ python3-venv is already installed")
    except subprocess.CalledProcessError:
        print("Installing python3-venv...")
        try:
            subprocess.run(['sudo', 'apt', 'update'], check=True)
            subprocess.run(['sudo', 'apt', 'install', '-y', 'python3.8-venv'], check=True)
...
def install_python_dependencies(venv_dir):
    ...
    try:
        ...
        subprocess.run([str(pip_path), 'install', 'python-telegram-bot==20.5'], check=True)
...
def get_user_input():
    ...
    config['BOT_TOKEN'] = input("Enter BOT_TOKEN (e.g., 8409119937:AAGsRqdyg-U_caPKsDUpnhzMM8SyG57Qqw): ").strip()
    config['ADMIN_ID'] = input("Enter ADMIN_ID (numeric Telegram ID): ").strip()
    config['ADMIN_PIN'] = input("Enter ADMIN_PIN (for admin login): ").strip()
    config['ADMIN_USERNAME'] = input("Enter ADMIN_USERNAME (e.g., @juevpn): ").strip()
    
    return config

def create_environment_file(config):
    """Create environment file"""
    env_file = Path('/etc/default/telegram-bot')
...
"""
    ...
    try:
        ...
        subprocess.run(['sudo', 'chmod', '600', str(env_file)], check=True)
        print("✓ Environment file created")
    except Exception as e:
        print(f"✗ Failed to create environment file: {e}")
        
        # Create local .env file instead
        local_env = Path.home() / '.telegram-bot.env'
        local_env.write_text(env_content)
        print(f"✓ Created local environment file at {local_env}")
        return local_env
...
def create_bot_file():
    ...
    print(f"\nCreating bot.py file at {bot_file}...")
    try:
        bot_file.write_text(BOT_CODE)
        
        # Make executable
        bot_file.chmod(0o755)
        print("✓ bot.py file created")
        return bot_file
    except Exception as e:
        print(f"✗ Failed to create bot.py file: {e}")
        sys.exit(1)

def create_systemd_service(venv_dir, bot_file):
    """Create and start systemd service"""
    service_file = Path('/etc/systemd/system/telegram-bot.service')
    
    print("\nCreating systemd service...")
    
    # Ask for username
    username = input("Enter Linux username to run the service (default: current user): ").strip()
    if not username:
...
"""
    ...
    try:
        ...
        subprocess.run(['sudo', 'systemctl', 'enable', 'telegram-bot'], check=True)
        ...
        # Show status
        print("\nService status:")
        subprocess.run(['sudo', 'systemctl', 'status', 'telegram-bot', '--no-pager'], check=False)
...
def run_bot_manual(venv_dir, bot_file):
    """Run bot manually"""
    print("\n" + "="*50)
    ...
    os.environ['ADMIN_ID'] = config['ADMIN_ID']
    os.environ['ADMIN_PIN'] = config['ADMIN_PIN']
    os.environ['ADMIN_USERNAME'] = config['ADMIN_USERNAME']
    
    # Activate virtual environment and run bot
    print(f"\nActivating virtual environment and running bot...")
...
def main():
    """Main function"""
    print("="*60)
    print("TELEGRAM BOT AUTO INSTALLER")
    print("="*60)
    
    # Check if running as root
    if os.geteuid() == 0:
        print("Warning: Running as root. It's better to run as normal user.")
    
    # Ask for installation type
    print("\nInstallation Options:")
    print("1. Full automatic setup (recommended)")
    print("2. Manual setup and run")
    print("3. Exit")
    ...
    if choice == '1':
        ...
        bot_file = create_bot_file()
        create_systemd_service(venv_dir, bot_file)
        
        print("\n" + "="*60)
        print("SETUP COMPLETED!")
        print("="*60)
        print(f"\nYour bot is now running as a systemd service.")
        print(f"Check status: sudo systemctl status telegram-bot")
        print(f"View logs: sudo journalctl -u telegram-bot -f")
        print(f"\nBot files:")
        print(f"  - Bot script: {bot_file}")
        ...
        install_python_dependencies(venv_dir)
        bot_file = create_bot_file()
        run_bot_manual(venv_dir, bot_file)
